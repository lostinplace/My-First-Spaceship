...
public class OculusSpatializerFMOD : MonoBehaviour 
{
    ...
    void Start()
    {
        ...
        OSP_FMOD_AssignRayCastCallback(_raycastCallback, System.IntPtr.Zero);
    }

    void OnDestroy()
    {
        OSP_FMOD_AssignRayCastCallback(System.IntPtr.Zero, System.IntPtr.Zero);
    }
    ...
    void Update()
    {
        ...
        OSP_FMOD_SetDynamicRoomRaysPerSecond(raysPerSecond);
        OSP_FMOD_SetDynamicRoomInterpSpeed(roomInterpSpeed);
        OSP_FMOD_SetDynamicRoomMaxWallDistance(maxWallDistance);
        OSP_FMOD_SetDynamicRoomRaysRayCacheSize(rayCacheSize);

        gLayerMask = layerMask;
        OSP_FMOD_UpdateRoomModel(1.0f);
        ...
        if (visualizeRoom)
        {
            ...
            Vector3 pos;
            OSP_FMOD_GetRoomDimensions(dims, coefs, out pos);
            ...
            OSP_FMOD_GetRaycastHits(points, normals, HIT_COUNT);
	...
	// * * * * * * * * * * * * *
    // Import functions
    public delegate void AudioRaycastCallback(Vector3 origin, Vector3 direction, out Vector3 point, out Vector3 normal, System.IntPtr data);
	private const string strOSP = "OculusSpatializerFMOD";
    
    [DllImport(strOSP)]
    private static extern int OSP_FMOD_AssignRayCastCallback(System.MulticastDelegate callback, System.IntPtr data);
    [DllImport(strOSP)]
    private static extern int OSP_FMOD_AssignRayCastCallback(System.IntPtr callback, System.IntPtr data);
    [DllImport(strOSP)]
    ...
    [DllImport(strOSP)]
    private static extern int OSP_FMOD_UpdateRoomModel(float wetLevel); // call from main thread!!
    [DllImport(strOSP)]
